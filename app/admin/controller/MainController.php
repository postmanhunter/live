...
class MainController extends AdminbaseController {
    ...
    public function index(){
		...
		//小店商品待审核数量
		$stayinfo['shopgoods_count'] = Db::name('shop_goods')->where("status=0")->count();
		
		
		//小店商品退款需平台接入待处理数量
		$stayinfo['shop_order_refund_count'] = Db::name('shop_order_refund')->where("is_platform_interpose=1 and platform_result=0")->count();
		
		
		//付费内容待审核数量
		$stayinfo['paidprogram_count'] = Db::name('paidprogram')->where("status=0")->count();
		
		
		//付费内容申请待审核数量
		$stayinfo['paidprogram_applylist_count'] = Db::name('paidprogram_apply')->where("status=0")->count();
