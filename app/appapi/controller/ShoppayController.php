 ...
 * 商城订单支付回调
 */
class ShoppayController extends HomebaseController {
	...
	public function notify_ali() {
		...
		if($verify_result) {//验证成功
			...
			if($_POST['trade_status'] == 'TRADE_FINISHED') {
                ...
                $res=handelShopOrder($where,$data);
                ...
                if($res==-1){
                    $this->logali("orderno:".$out_trade_no.' 订单已关闭');	
                    echo "success";
                    exit;
				}
	...
	private function orderServer(){
        ...
        $where['total']=$info['total_fee']/100;
        ...
        if($res==-1){
            $this->logwx("orderno:".$out_trade_no.' 订单已关闭');	
            return false;
        }
    ...
    public function notify_wx_mini(){
		...
		if($arrayInfo['return_code'] == "SUCCESS"){
			...
			// }else{
				...
				ksort($arrayInfo);//按照字典排序参数数组
				$sign = $this -> sign($arrayInfo,$key);//生成签名
				$this -> logwx_mini("数据打印测试签名signmy:".$sign.":::微信sign:".$wxSign);//log打印保存
				if($this -> checkSign($wxSign,$sign)){
					echo $this -> returnInfo("SUCCESS","OK");
					$this -> logwx_mini("签名验证结果成功:".$sign);//log打印保存
					$this -> wxMiniOrderServer();//订单处理业务逻辑
					exit;
				}else{
					echo $this -> returnInfo("FAIL","签名失败");
    ...
    private function wxMiniOrderServer(){
        ...
        if($res==-1){
            $this->logwx_mini("orderno:".$out_trade_no.' 订单已关闭');	
            return false;
        }
	...
	*/
	...
	public function logali($msg){
		file_put_contents(CMF_ROOT.'data/shoppaylog/logali_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg.PHP_EOL.PHP_EOL,FILE_APPEND);
	}		
	/* 打印log */
	public function logwx($msg){
		file_put_contents(CMF_ROOT.'data/shoppaylog/logwx_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg.PHP_EOL.PHP_EOL,FILE_APPEND);
	}

	/* 打印log */
	public function logwx_mini($msg){
		file_put_contents(CMF_ROOT.'data/shoppaylog/logwx_mini_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg.PHP_EOL.PHP_EOL,FILE_APPEND);
	}			
