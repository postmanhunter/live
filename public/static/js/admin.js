;(function () {
    ...
    //文字提示图片效果
    var msg_tips=$('.msg_tips');
    var html_tip='<div id="enlarge_images" style="position:fixed;display:none;z-index:2;"></div>';
    msg_tips.parents('body').append(html_tip);
    var ei=msg_tips.parents('body').find('#enlarge_images');
    if(ei.length>0){
        msg_tips.mousemove(function(event){
            event = event || window.event;
            ...
            img.onload = function(){
                ...
                //console.log("图片高度："+img_height);

                var html = '<img src="' + msg_url + '" style="max-width:800px;max-height:800px;"/>';
            
                ei.html(html);
                // var top  = document.body.scrollTop + event.clientY + 10 + "px";
                // var left = document.body.scrollLeft + event.clientX + 10 + "px";
                //var top  = document.body.scrollTop + event.clientY + 10;
                var top= $(document.body).height()-event.clientY-$(document).scrollTop();
                var left = document.body.scrollLeft + event.clientX + 10+'px';
