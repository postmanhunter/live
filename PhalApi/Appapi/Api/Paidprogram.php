...
class Api_Paidprogram extends PhalApi_Api {
	...
	public function getApplyStatus(){
        ...
        $rs['info'][0]['isauth']='1';
        $rs['info'][0]['auth_msg']='申请开通付费内容权限需要先进行实名认证';
	...
	public function getBalance(){
        ...
        if($paidprogram_paypal_switch){
            $paylist[]=[
                'id'=>'paypal',
                'name'=>'Paypal支付',
                'thumb'=>get_upload_path("/static/app/paidprogrampay/paypal.png"),
                'href'=>'',
                'type'=>'5'
            ];
        }*/
	...
	*/
	...
	private function getPaypalOrderBF(){
		...
		$rs['info'][0]=$paypal;
		$rs['info'][0]['orderid']=$orderid;
		$rs['info'][0]['goods_name']=filterEmoji($paidprogram_info['title']);
		return $rs;
	}
