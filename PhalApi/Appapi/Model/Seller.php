...
class Model_Seller extends PhalApi_Model_NotORM {
    ...
    public function getGoodsNums($uid){
        ...
        $where=[];
        $where['uid']=$uid;
        $platform_num=$model_shop->countPlatformSale($where);
        if($platform_num){
            $res['platform']=$platform_num;
        }
    ...
    public function getSettlementList($uid,$p){
        ...
        $list=DI()->notorm->user_balance_record
                ->where("uid=? and type=1 and (action =2 or action=9)",$uid)
                ->order("addtime desc")
                ->limit($start,$pnums)
                ->fetchAll();
    ...
    public function getPlatformGoodsLists($uid,$p){
        ...
        $where['uid']=1;
        $where['status']=1; 
        $where['type']=2; //商品类型为平台自营商品

        $list=handleGoodsList($where,$p);

        foreach ($list as $k => $v) {
            $isadd=$this->checkSellerIsBehalf($uid,$v['id']);
            $list[$k]['isadd']=$isadd;
        }
    ...
    //判断卖家是否代卖了平台商品
    public function checkSellerIsBehalf($uid,$goodsid){
    ...
    public function setPlatformGoods($uid,$goodsid){
        ...
        if($info){
          ...
          if(!$res){
            ...
            //判断商品是否已经下架
            $model_shop=new Model_Shop();
            $goods_info=$model_shop->getGoods(['id'=>$goodsid]);
           ...
           $res=DI()->notorm->seller_platform_goods->insert($data);
           if(!$res){
                return 1002;
           }
    ...
    public function getOnsalePlatformGoods($uid,$p){
        ...
        if($p>1){
            $seller_platform_addtime=$_SESSION['seller_platform_addtime'];
            if($seller_platform_addtime){
                $where.=" and addtime<".$seller_platform_addtime;
            }  
        }
        ...
        $end=end($platform_list);
        if($end){
            $_SESSION['seller_platform_addtime']=$end['addtime'];
        }
