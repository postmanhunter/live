...
class Model_Cdnrecord extends PhalApi_Model_NotORM {
	...
	public function getCdnRecord($id) {
		...
		$result=DI()->notorm->live_record->select('id,starttime,endtime,stream,video_url')->where('id=?',$id)->fetchOne();
    ...
    protected function alicdn($data){
        ...
        DI()->notorm->live_record->where('id=?',$data['id'])->update( array('video_url'=>$video_url) );
