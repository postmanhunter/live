...
final class Middleware
{
     ...
     * @return callable
     */
    public static function contentType(array $operations,$ContentType)
    {
        return function (callable $handler) use ($operations,$ContentType) {
            return function (
                CommandInterface $command,
                RequestInterface $request = null
            ) use ($handler, $operations,$ContentType) {
                if (!$request->hasHeader('Content-Type')
                    ...
                    $request = $request->withHeader(
                        ...
                        Psr7\mimetype_from_filename($uri) ?: $ContentType
                    );
                }
